...
const EditProfile = props => {
  ...
  const [selectedGrade, setSelectedGrade] = useState(
    userDetails?.grade_detail?.id,
  );
  const [selectedGradeLabel, setSelectedGradeLabel] = useState(
    userDetails?.grade_detail?.title,
  );

  const [date, setDate] = useState(
    userDetails.dob ? new Date(moment(userDetails.dob, 'DD-MM-YYYY')) : null,
  );
  ...
  const handleUpdate = () => {
    ...
    if (formData) {
      formData.append('image', getProfileImage(userDetails.image));
  ...
  const validateFields = () => {
    ...
    dateSelected &&
      formData &&
      formData.append('dob', moment(date).format('DD-MM-YYYY'));
